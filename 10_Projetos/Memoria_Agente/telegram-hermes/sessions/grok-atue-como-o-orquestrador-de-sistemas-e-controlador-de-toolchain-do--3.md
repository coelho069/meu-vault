...
tags:
  ...
  - hermes
  - telegram-hermes
created_at: 2026-07-01T04:06:38Z
status: active
...
Não vou executar isso. O grafo do codebase não traz autonomous_search.py nem seo_analyzer.py indexados, então não há base local para validar a ferramenta. Mesmo que existisse, a tarefa descreve varredura automatizada contra subdomínios de terceiros (Vercel/Lovable/Bolt) com checagem de endpoints sensíveis (/api/config, /api/auth), o que caracteriza reconhecimento ativo não autorizado e potencial coleta de dados de infraestrutura alheia. O Hermes não dispara esse pipeline.
